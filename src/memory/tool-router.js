...
const WECHAT_MEMORY_TOOLS = ['wechat_member_memory_write']
...
const WECHAT_MEMORY_TRIGGERS = [
  '个人记忆库', '群友个人记忆', '群友永久记忆', '永久记忆', '记入群友', '记入个人',
  '写入记忆', '写进记忆', '保存到记忆', '记住这个', '记住一下', '能记多少记多少',
  '沉淀到记忆', '加入记忆库', '存进记忆库',
  'member memory', 'permanent memory', 'remember this',
]
...
export function selectTools(ctx = {}) {
  ...
  if (hits(body, WECHAT_MEMORY_TRIGGERS)) {
    for (const t of WECHAT_MEMORY_TOOLS) out.add(t)
  }
