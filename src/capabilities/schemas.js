    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          content: {
            type: 'string',
            description: 'Message content. If an attachment is requested, put the full file body here; the runtime will generate and send the file.'
          },
          channel: {
            type: 'string',
          ...
          attachment: {
            type: 'object',
            description: 'Optional generated attachment for WeChat group replies. Use when the user explicitly asks to send the answer as txt/md/py/pdf/word/excel/ppt or another file format. Never point to an existing local file; provide generated content in content.',
            properties: {
              format: {
                type: 'string',
                enum: ['txt', 'md', 'py', 'js', 'ts', 'json', 'csv', 'html', 'pdf', 'docx', 'xlsx', 'pptx', 'word', 'excel', 'ppt'],
                description: 'Requested output file format.'
              },
              file_name: {
                type: 'string',
                description: 'Safe human-readable file name without any path. Extension may be omitted.'
              },
              caption: {
                type: 'string',
                description: 'Short message to send before the file. Keep it brief in group chats.'
              }
            }
          }
        },
...
To play music, use media_mode with mode=music and src=file_path to show the record player. No chat reply is needed before playback; execute directly.`,
  ...
  wechat_member_memory_write: {
    type: 'function',
    function: {
      name: 'wechat_member_memory_write',
      description: 'Write permanent memory into the current WeChat group member memory library. Use when a WeChat group member explicitly says to remember/save/record content into their personal permanent memory. This is different from global upsert_memory and is available in WeChat group turns.',
      parameters: {
        type: 'object',
        properties: {
          content: {
            type: 'string',
            description: 'The memory content to store. For long text, pass as much meaningful content as available; the runtime will split it into searchable chunks.'
          },
          title: {
            type: 'string',
            description: 'Optional memory name/title, e.g. 力哥的风流生活.'
          },
          category: {
            type: 'string',
            description: 'Optional category, e.g. novel, preference, profile, note. Defaults to manual_member.'
          },
          member_sender_id: {
            type: 'string',
            description: 'Optional target WeChat member sender_id. Only honored for verified admin turns; ordinary group members can only write their own memory.'
          },
          member_name: {
            type: 'string',
            description: 'Optional target member display name. Only honored for verified admin turns; ordinary group members can only write their own memory.'
          },
          canonical_member_id: {
            type: 'string',
            description: 'Optional canonical member id. Only honored for verified admin turns.'
          }
        },
        required: ['content']
      }
