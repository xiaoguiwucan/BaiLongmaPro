...
import { createWechatReplyFile, defaultWechatReplyFileCaption, hasExplicitWechatReplyFileFormatRequest, isWechatReplyAcknowledgementOnly, isWechatSubstantiveReplyRequest, shouldAutoCreateWechatReplyFile } from '../social/wechat-file-reply.js'
import { createWeChatGroupManualMemory } from '../social/wechat-group-memory.js'
...
const TOOL_RISK = {
  ...
  wechat_member_memory_write: 'medium',
...
function summarizeToolExecution(name, args = {}) {
  switch (name) {
    ...
    case 'wechat_member_memory_write':
      return `wechat_member_memory_write(${String(args.title || args.category || '').slice(0, 60) || 'member memory'})`
    default:
      return name
  }
}
...
async function executeToolUnchecked(name, args, context = {}) {
  try {
    ...
    switch (name) {
      ...
      case 'wechat_member_memory_write':
        return await execWechatMemberMemoryWrite(args, context)
...
async function execSendMessage({ target_id, content, channel = 'AUTO', attachment = null }, context = {}) {
  ...
  const wechatUserText = String(social.user_text || social.raw_user_text || '').trim()
  if (social.platform === 'wechaty-duty-group' && isWechatSubstantiveReplyRequest(wechatUserText) && isWechatReplyAcknowledgementOnly(cleanedContent)) {
    return '错误：微信群这条请求需要实际内容，不能只回复“好的/稍等/马上/接上”。请在本轮直接生成完整正文；如果用户明确要求 txt/md/pdf/word/excel/ppt/代码等文件格式，还必须用 send_message.attachment 发送生成文件。'
  }
  ...
  let outboundContent = cleanedContent
  const generatedFiles = []
  const requestedAttachment = attachment && typeof attachment === 'object' ? attachment : null
  const explicitFileRequest = social.platform === 'wechaty-duty-group'
    ? hasExplicitWechatReplyFileFormatRequest(wechatUserText)
    : false
  const autoAttachment = !requestedAttachment && delivery.deliveryChannel === 'WECHAT' && social.platform === 'wechaty-duty-group'
    ? shouldAutoCreateWechatReplyFile({ userText: wechatUserText, content: cleanedContent })
    : { create: false }
  const shouldGenerateWechatAttachment = delivery.deliveryChannel === 'WECHAT'
    && social.platform === 'wechaty-duty-group'
    && explicitFileRequest
    && (requestedAttachment || autoAttachment.create)
  if (shouldGenerateWechatAttachment) {
    try {
      const generated = await createWechatReplyFile({
        content: cleanedContent,
        format: requestedAttachment?.format || requestedAttachment?.file_format || autoAttachment.format || '',
        fileName: requestedAttachment?.file_name || requestedAttachment?.filename || requestedAttachment?.name || '',
        title: requestedAttachment?.title || requestedAttachment?.file_name || '',
      })
      generatedFiles.push(generated.filePath)
      const caption = String(requestedAttachment?.caption || '').trim()
      outboundContent = caption || defaultWechatReplyFileCaption(generated)
    } catch (err) {
      return `错误：生成微信群附件失败：${err?.message || err}`
    }
  }
  ...
  console.log(`  ${outboundContent}${generatedFiles.length ? ` [附件 ${generatedFiles.length} 个]` : ''}`)
  ...
  insertConversation({
    ...
    from_id: 'jarvis',
    to_id: resolvedId,
    content: generatedFiles.length ? `${outboundContent}\n[生成附件正文]\n${cleanedContent}` : cleanedContent,
    timestamp,
  ...
  if (!delivery.isLocal && delivery.externalTargetId) {
    try {
      socialResult = await dispatchSocialMessage(delivery.externalTargetId, outboundContent, { social: context.currentSocial || null, filePaths: generatedFiles })
    } catch (err) {
...
function getCommandFailureHint(command = '', stderr = '', stdout = '') {
  const text = `${stderr || ''}\n${stdout || ''}`
  if (SHELL_FILE_WRITE_RE.test(command) && ESCAPE_FAILURE_RE.test(text)) {
    return 'This failed because file content was written through the shell and PowerShell mangled quotes/$/backticks/triple-quotes. Do not retry with different escaping. Use write_file with { path, content } and pass the full file body verbatim.'
  }
  if (/\bssh\b/i.test(command) && /syntax error:\s*unexpected end of file/i.test(text)) {
    return 'The remote shell command reached bash with broken quoting or an unfinished block. Simplify the remote command or fix quote escaping before retrying.'
  }
  if (/\bssh\b/i.test(command) && /unexpected EOF while looking for matching/i.test(text)) {
    return 'SSH likely connected, but the remote command quoting was unbalanced. Fix the quote escaping before retrying.'
  }
...
function splitWechatMemberMemoryContent(content = '', maxChars = 3500) {
  const text = String(content || '').replace(/\r\n/g, '\n').replace(/\r/g, '\n').trim()
  if (!text) return []
  const max = Math.max(800, Math.min(Number(maxChars || 3500), 6000))
  if (text.length <= max) return [text]
  ...
  for (const paragraph of paragraphs.length ? paragraphs : [text]) {
    if (paragraph.length > max) {
      push()
      for (let i = 0; i < paragraph.length; i += max) chunks.push(paragraph.slice(i, i + max).trim())
      continue
    }
...
async function execWechatMemberMemoryWrite(args = {}, context = {}) {
  const social = context.currentSocial || {}
  if (social.platform !== 'wechaty-duty-group' || !social.room_id) {
    return JSON.stringify({ ok: false, error: 'wechat_member_memory_write 只能在微信群助手回合中使用' })
  }
  const content = String(args.content || '').trim()
  if (!content) return JSON.stringify({ ok: false, error: '记忆内容不能为空' })
  const admin = social.wechat_admin === true
  const groupId = `wechaty:${social.room_id}`
  const groupName = social.group_name || ''
  const senderId = admin
    ? String(args.member_sender_id || args.sender_id || social.sender_id || '').trim()
    : String(social.sender_id || '').trim()
  const senderName = admin
    ? String(args.member_name || args.sender_name || social.sender_name || '').trim()
    : String(social.sender_name || '').trim()
  const canonicalMemberId = admin ? String(args.canonical_member_id || args.canonicalMemberId || '').trim() : ''
  const title = String(args.title || args.name || '').trim().slice(0, 120)
  const category = String(args.category || 'manual_member').trim().slice(0, 80) || 'manual_member'
  const chunks = splitWechatMemberMemoryContent(content, args.chunk_chars || args.chunkChars || 3500)
  if (!chunks.length) return JSON.stringify({ ok: false, error: '记忆内容为空' })

  const results = []
  for (let i = 0; i < chunks.length; i++) {
    const prefix = title
      ? chunks.length > 1 ? `【${title}】第 ${i + 1}/${chunks.length} 部分\n` : `【${title}】\n`
      : chunks.length > 1 ? `第 ${i + 1}/${chunks.length} 部分\n` : ''
    const result = await createWeChatGroupManualMemory({
      ...
      senderName,
      canonicalMemberId,
      content: `${prefix}${chunks[i]}`.trim(),
      category,
    })
    results.push({
      ok: result?.ok === true,
      deduped: result?.deduped === true || result?.local?.deduped === true,
      ids: Array.isArray(result?.items) ? result.items.map(item => item.id).filter(Boolean) : [],
      error: result?.error || result?.local?.error || '',
    })
  }
  const errors = results.filter(item => !item.ok && item.error).map(item => item.error)
  return JSON.stringify({
    ok: errors.length === 0,
    tool: 'wechat_member_memory_write',
    group_id: groupId,
    group_name: groupName,
    member_sender_id: senderId,
    member_name: senderName,
    title,
    category,
    chunks: chunks.length,
    written: results.filter(item => item.ok).length,
    errors,
    note: '已写入当前微信群的群友个人永久记忆库；长文本已分块，方便关键词和向量召回。',
  })
}
