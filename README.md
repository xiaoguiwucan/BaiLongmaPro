...
**v0.4.98** | 一个持续运行、能记忆、能发声、能接入微信群和本地工具的数字意识实验框架。
...
`v0.4.98` 新增微信群组专用备份与迁移，并修复多模态 Skill 真实模型测试：可按群导出聊天流水、图片解析、群记忆、成员记忆和成员身份映射；导入前会用当前在线微信号实时群列表做安全匹配，生图/识图/视频测试会复用真实运行链路并给出清晰错误诊断。
...
- `v0.4.98`：新增微信群组备份与迁移中心；备份只包含微信群数据白名单，不包含 LLM 渠道、Skill、知识库、密钥、token、embedding 和 Honcho 同步字段；导入按实时群列表精确匹配或经确认的唯一群名匹配，并支持合并去重、媒体恢复和 FTS 重建；多模态 Skill 模型测试改为真实生图/识图/视频链路并增强 UI 反馈。
- `v0.4.95`：修复 GitHub Actions Release 发布流程并发创建同一 GitHub Release 的问题；Windows 与 macOS 发布矩阵改为串行执行，避免 `already_exists field=tag_name` 422 错误。
- `v0.4.94`：默认源码启动改为 LAN 模式；`npm start`、`npm run start:backend` 与原有 LAN 别名统一走跨平台启动器，监听 `0.0.0.0` 并打印局域网访问地址。
...
默认源码启动已经启用 LAN：后端监听 `0.0.0.0:3721`，启动器会打印当前机器的 `http://<局域网 IP>:3721/brain-ui`。如果同网设备打不开页面，优先检查系统防火墙是否允许 Node/Electron 访问专用网络。
...
| `scripts/start-lan.mjs` | 默认跨平台 LAN 启动器 |
| `scripts/start-lan.ps1` | Windows PowerShell LAN 启动脚本 |
