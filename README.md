...
**v0.4.96** | 一个持续运行、能记忆、能发声、能接入微信群和本地工具的数字意识实验框架。
...
`v0.4.96` 新增微信群组专用备份与迁移：可按群导出聊天流水、图片解析、群记忆、成员记忆和成员身份映射；导入前会用当前在线微信号实时群列表做安全匹配，避免把不属于当前账号的群数据导入本机。
...
- `v0.4.96`：新增微信群组备份与迁移中心；备份只包含微信群数据白名单，不包含 LLM 渠道、Skill、知识库、密钥、token、embedding 和 Honcho 同步字段；导入按实时群列表精确匹配或经确认的唯一群名匹配，并支持合并去重、媒体恢复和 FTS 重建。
