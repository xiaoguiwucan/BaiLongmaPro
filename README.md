...
**v0.4.95** | 一个持续运行、能记忆、能发声、能接入微信群和本地工具的数字意识实验框架。
...
`v0.4.95` 修复 GitHub Actions Release 发布流程：Windows 与 macOS 资产会串行上传到同一个 GitHub Release，避免并发创建同一 tag 时出现 `already_exists field=tag_name` 失败。
