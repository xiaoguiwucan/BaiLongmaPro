...
未发布版本继续强化微信群助手：新增非 @ 主动回复、屏蔽成员、左侧二级设置菜单和舆情推送；群聊总结改为 2x 高清手机长图，包含一句话总结、主要话题证据、关键时间线、活跃成员和干货总结。`v0.4.92` 已深修微信群长文/文件附件回复链路，确保“3000 字，用文本文档格式发给我”等请求不会只回“好的/稍等”后静默结束。
...
- `未发布 - 2026-06-04`：微信群助手新增非 @ 主动回复、屏蔽成员和左侧二级设置菜单；群聊总结长图改为 2x 高清输出并补充话题证据、关键时间线和干货总结；新增舆情变动微信群推送，优先发送 PNG 海报；项目规范改为所有变更历史统一写入 `CHANGELOG.md`，新增功能需同步更新 README，Git 提交信息必须使用中文。
